...
def reset_deposit(client: FlaskClient, token):
    return client.patch('/reset', headers={'Authorization': token})


def deposit(client: FlaskClient, deposit, token):
    return client.patch('/deposit', json={"deposit": deposit}, headers={'Authorization': token})
...
def test_reset_deposit_ok(client: FlaskClient):
    ...
    res = reset_deposit(client, token)
    assert res.status_code == HTTPStatus.OK
...
def test_deposit_conflict(client: FlaskClient):
    token = get_token_for_user(client, 'buyer')
    res = deposit(client, 33, token)
    assert res.status_code == HTTPStatus.CONFLICT


def test_buy_ok(client: FlaskClient):
    token = get_token_for_user(client, 'buyer')
    reset_deposit(client, token)
    deposit(client, 100, token)
    deposit(client, 50, token)
